...
const CONTACT_INFO = [
  ...
  {
    label: "Argentina — Disponible para viajar",
    href: null,
    icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const Contact = () => {
  return (
    <footer
      id="contacto"
      className="bg-[#080808] text-white border-t border-white/10"
    >
      {/* Main footer */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2 mb-4">
              <span className="w-1.5 h-8 rounded-full bg-[#94e115]" />
            ...
            <div className="flex gap-3">
              {SOCIAL_LINKS.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:bg-[#94e115] hover:text-black hover:border-[#94e115] transition-all duration-300"
                >
                  {s.icon}
                </a>
              ))}
            ...
            <ul className="space-y-3">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-gray-500 text-sm hover:text-[#94e115] transition-colors duration-200 flex items-center gap-2 group"
                  >
                    <span className="w-3 h-px bg-gray-700 group-hover:bg-[#94e115] group-hover:w-4 transition-all duration-200" />
                    {link.label}
                  </a>
                </li>
              ))}
              ...
              {CONTACT_INFO.map((item, i) =>
                item.href ? (
                  <li key={i}>
                    <a
                      href={item.href}
                      className="flex items-start gap-3 text-gray-500 text-sm hover:text-[#94e115] transition-colors duration-200 group"
                    >
                      <span className="text-[#94e115] mt-0.5 flex-shrink-0">
                        {item.icon}
                      </span>
                      {item.label}
                    </a>
      ...
      <div className="border-t border-white/5">
        <div className="container mx-auto px-4 max-w-7xl py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-gray-600 text-xs">
            © {new Date().getFullYear()} Training Wraps. Todos los derechos
            reservados.
          </p>
          <p className="text-gray-700 text-xs flex items-center gap-1">
            Hecho con{" "}
            <span className="text-[#94e115]">💚</span>{" "}
            para la comunidad del wrapping
          </p>
        </div>
      </div>
    </footer>
